...
from app.tools.sharepoint_tool import find_sharepoint_files, search_sharepoint
...
@pytest.mark.asyncio
async def test_find_sharepoint_files_uses_azure_function_by_default(monkeypatch):
    monkeypatch.delenv("SHAREPOINT_TOOL_BACKEND", raising=False)
    monkeypatch.setenv("SHAREPOINT_FUNCTION_URL", "https://func.example.azurewebsites.net")

    with respx.mock:
        route = respx.post("https://func.example.azurewebsites.net/files/find").mock(
            return_value=httpx.Response(200, json={"files": [{"doc_id": "1"}]})
        )
        result = await find_sharepoint_files(
            library="Documents", file_name_pattern="*report*", site_url="https://s.example.com"
        )

    assert route.called
    request_body = route.calls.last.request.content
    assert b'"library":"Documents"' in request_body
    assert b'"file_name_pattern":"*report*"' in request_body
    assert result == [{"doc_id": "1"}]


@pytest.mark.asyncio
async def test_find_sharepoint_files_python_backend_explicit(monkeypatch):
    monkeypatch.setenv("SHAREPOINT_SERVICE_URL", "https://sidecar.example.com")

    with respx.mock:
        route = respx.post("https://sidecar.example.com/files/find").mock(
            return_value=httpx.Response(200, json={"files": [{"doc_id": "2"}]})
        )
        result = await find_sharepoint_files(
            library="Documents",
...
@pytest.mark.asyncio
async def test_find_sharepoint_files_raises_when_azure_function_url_unset(monkeypatch):
    monkeypatch.delenv("SHAREPOINT_FUNCTION_URL", raising=False)

    with pytest.raises(NotImplementedError):
        await find_sharepoint_files(
            library="Documents",
            file_name_pattern="*report*",
            site_url="https://s.example.com",
            backend="azure_function",
        )


@pytest.mark.asyncio
async def test_find_sharepoint_files_invalid_backend_raises_value_error():
    with pytest.raises(ValueError, match="invalid_backend_name"):
        await find_sharepoint_files(
            library="Documents",
            file_name_pattern="*report*",
